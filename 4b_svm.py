# coding: utf-8
# 4b_svm.py
# Step 4b: SVM (RBF kernel) classifier on HSI band-selected data
# LOPOCV (Leave-One-Patient-Out), 3 folds x 16 combos = 48 runs
# StandardScaler fitted on train fold only (never leaks to test).
# Results auto-saved after every fold; supports checkpoint/resume.
#
# WARNING: FullSpectrum fold (699 bands) may take 30-60+ minutes per fold
...
def main():
    print('=' * 60)
    print('WARNING: FullSpectrum fold (699 bands) may take 30-60+ min')
    print('         due to SVM O(n^2) training complexity.')
    print('         Estimated total runtime: 1.5 - 3 hours.')
    print('         Checkpoint/resume is active - safe to interrupt.')
    print('=' * 60)
    print()
    ...
    print('SVM v1  |  {} combos x {} folds = {} runs'.format(
