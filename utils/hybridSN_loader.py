...
def get_fold_patches_split(fold_dict, band_indices, patch_size=11,
    ...
    Uses seed for train, seed+1 for test to guarantee different random patches.
    ...
    test_dict  = {'train_files': fold_dict['test_files']}

    X_train, y_train = get_fold_patches(
        train_dict, band_indices, patch_size=patch_size,
        patches_per_roi=patches_per_roi, seed=seed
    )
    X_test, y_test = get_fold_patches(
        test_dict, band_indices, patch_size=patch_size,
        patches_per_roi=patches_per_roi, seed=seed + 1
    )
